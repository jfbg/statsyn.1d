% Load list of runs in mat4_averagelist.list and compile the listed files.
% Assumes  distance range is 0-180

fid = fopen('matGCC_averagelist.list','r');
run_names = textscan(fid,'%s','delimiter','\n');
run_names = run_names{1};

fid = fopen('matGCC_outputs.list','r');
run_out = textscan(fid,'%s','delimiter','\n');
run_out = run_out{1};

dtemp = load('matGCC_dt.list');
