...
!	Modified from Jesse Lawrence's inter_mod.f by JFBG (Oct 2011)
      ...
      INTEGER       NMX
      PARAMETER(    NMX = 10000)
      REAL          z(NMX),r(NMX),vp(NMX),vs(NMX),rh(NMX),Q(NMX)
      CHARACTER*80  ifile
      INTEGER       status
      REAL          zz,rr,pp,ss,hh,QQ
      ...
      DO I = 1, NMX                     !READ IN VELOCITY MODEL
       READ(1,*,IOSTAT=status) z(I),r(I),vp(I),vs(I),rh(I),Q(I)
...
!			WRITE(6,*) nlyr
      ...
      DO I = 1, nlyr-1                 !FOR EACH LAYER:
       IF (z(I).LT.z(I+1)) THEN        !CHECK VELOCITY LAYER (NOT INTERFACE)
					...
					ENDIF
        WRITE(2,FMT=1010) z(I),r(I),vp(I),vs(I),rh(I),Q(I)!OUTPUT TOP VELOCITY
        DO J = 1, nl-1                 !INTERPOLATE AT EACH STEP
         ...
         hh = rh(I) + float(J)*(rh(I+1)-rh(I))/float(nl)
				 QQ = Q(I) + float(J)*(Q(I+1)-Q(I))/float(nl)
         WRITE(2,FMT=1010) zz,rr,pp,ss,hh,QQ !OUTPUT INTERPOLATION
        END DO                         !
       ELSE                            !IF INTERFACE JUST OUTPUT THE INPUT
        WRITE(2,FMT=1010) z(I),r(I),vp(I),vs(I),rh(I),Q(I)
       END IF
      END DO
      I = nlyr
      WRITE(2,FMT=1010) z(I),r(I),vp(I),vs(I),rh(I),Q(I)
      
1010  FORMAT(5(2X,F10.5),2X,F9.2)             !OUTPUT FORMAT
      
      CLOSE(2)
      
      
      STOP
      END PROGRAM interp_modelQ
