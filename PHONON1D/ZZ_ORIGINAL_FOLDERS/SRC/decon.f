      ...
      DO I = 1, nt                                     !FOR EACH TIME & DNSTANCE
       ...
       IF (status /= 0) STOP                           !IF READ ERROR, THEN STOP
      END DO                                           !
      ...
      DO I = 1, nd
       ...
       DO J = 1, nt                                    !FOR EACH POINT NEAR TRIGGER
        IF (J < i_trigger - n_win) THEN                !CHECK IF IN TRIGGER WINDOW
         s2(J) = 0.
	 CYCLE
        ELSE IF (J > i_trigger + n_win) THEN           !CHECK IF IN TRIGGER WINDOW
         s2(J) = 0.
         CYCLE
        ELSE
         t1 = float(J-i_trigger)*dt                     !TIME FROM TRIGGER
         wt = abs(cos(t1/win_length*pi/2.))             !WEIGHT FOR POINT
         s2(J) = s2(J) * wt                             !WEIGHT THE SERIES
        END IF
       END DO
...
C       WRITE(6,*) npts,dt
       CALL TRANSFER_JL2(npts,dt,tdelay,s2,s1,tf21)    !DECONVOLVE GET TRANSFER FUNCT
