...
long *width, *dash, *join, *cap;
{
  ...
  switch(*cap) {
	...
	case 3: c = CapProjecting;  break;  // Bugfix - rshearer
    default: c= CapButt;
  }
  switch(*join) {
...
/* additional methods written by rshearer */

void lfillpolygon_(x, y, npoints, shape, mode)
/*
Fills a polygon.  X and y are arrays of integers (x and y coordinates) of the points used to draw the polygon,
and npoints is the number of points.
Shape: 0: Complex (path may self-intersect)
       1: Noncovex (path has concave parts)
	   2: Convex (path is always convex)
Mode:  0: CoordModeOrigin (points relative to origin)
       1: CoordModePrevious (points after first relative to previous point)
*/
long *x, *y, *npoints, *shape, *mode;
{
  XPoint *xp;
  int s, m;
  long i, nxp;
  ...
  switch(*shape)
  {
    case 0: s= Complex;	break;
    case 2: s= Convex;	break;
    default: s= Nonconvex;
  }
  switch(*mode)
  {
    case 1: m= CoordModePrevious;	break;
    default: m= CoordModeOrigin;
  }
  ...
  XFillPolygon(display, draw, usergc, xp, nxp, s, m);
}
