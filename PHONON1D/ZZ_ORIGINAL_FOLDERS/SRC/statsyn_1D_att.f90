...
!
      ...
      real          qd1,qd2,qqq           !Anomalous Q depth range and value
...
60    write(6,'(A)') 'ENTER DISTANCE RANGE (x1, x2, nx IN DEGREES):'
      ...
      read (5,    *)  scat_prob
      write(6,'(A)') 'Enter q depth range & values (d1,d2,Q):'
      read (5,    *)  qd1,qd2,QQQ
...
22    FORMAT (f6.1,2(i5,f6.2,f5.2),2x,2f9.6,2x,2f9.6)
      ...
      do I = 1, nlay+5                      !BUILD Q(z) MODEL
       Q(I) = 10000.
       if ((z_s(I) > qd1).AND.(z_s(I)< qd2)) Q(I) = qqq
...
!        Q(I) = 3000.
!       end if
       if (EorM  ==  2) Q(I) = 12000       !FOR MOON Q IS HIGH UNTIL ??CORE??
      end do
	 ...
	 wf(I,J,k) = 0.
      ...
      do I = 1, ntr                           !FOR EACH TRACE
       ...
       az   = 0.
       do while ((p < p1).OR.(p > 1./vf(iz,iwave)))
        r0 = ran0(IDUM)                       !SELECT RANDOM RAY PARAMETER 
        ang1 = angst*r0
        p    = abs(sin(ang1))/vf(iz,iwave)
       end do
       ...
       do while ((t < t2).AND.(NITR < 10*nlay))!TRACE UNTIL TIME PASSES TIME WIND
        NITR = NITR + 1
...
!   |FIX NEXT if FOR DIFFRACTED WAVES: 
	...
	r0 = ran0(IDUM)
        ...
        if (iz == 1) then                      !IF RAY HITS SUFACE then RECORD
         ...
         if (abs(x/deg2km)-abs(x1+dxi*float(ix-1)) > 0.15) cycle
...
!          if (it>1)write(6,*) ip,iwave,ix,it,a,ang1*180/pi,c_mult(1),c_mult(2),c_mult(3)
          ...
          if((n_iter_last == niter).and.(ix_last==ix) &
