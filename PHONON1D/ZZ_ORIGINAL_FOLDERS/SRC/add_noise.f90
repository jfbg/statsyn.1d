program add_noise
      implicit     NONE
      integer, parameter :: SMX=200000,DMX=1000         !MAX SIG & DNST LENGTH
      ...
      write(2,FMT=888) blnk,(d(J),J=1,nd)              !write IN DISTANCES
      dx = (d(nd)-d(1))/(nd-1)
			write(6,*) d(nd),d(1),dx			!JFBG
...
!       write(6,*) I,tt(I)
      ...
      dt = tt(2)-tt(1)                                 !TIME INTERVAL
			write(6,'(I4,1X,I10,1X,2(F6.4,1X))') nd,nt,dx,dt			!JFBG
      ...
      do i = 1, nd
       do j = 1, nt
        if (abs(s_in(i,j)) > 99.) s_in(i,j)=s_in(i,j)/100.
			...
			write(6,'(a)') 'CHECK68'	!JFBG
      
      do J = 1, nt                                    !FOR EACH POINT NEAR TRIGGER
        if (maxf < abs(s_in(id,j))) then
					maxf = abs(s_in(id,j))
					!write(6,*) 'DING:',id,j,s_in(id,j),maxf
				endif
      end do
			
			maxf = 0.0682149976 !JFBG
...
888   format(F15.6,1X,500(F10.6,1X))                       !FORMATTING STATEMENT
      stop
