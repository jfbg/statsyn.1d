...
22    FORMAT (f6.1,2(i5,f6.2,f5.2),2x,2f9.6,2x,2f9.6)
			...
			nttrack = 2!nint(real(nt/nttrack_dt) - .5)	!-0.5 will make it round down
...
!         ims = 2
					...
					attn = (   (1.-frac)*mts(ims-1,icaust,1) &
                          + (   frac)*mts(ims  ,icaust,1) )
					...
					trackcount(ix,iz,itt) = trackcount(ixtrack,iztrack,itt) + 1		!+1 at phonon's location
				end if
