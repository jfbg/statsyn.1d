...
!			real	,allocatable,dimension(:,:,:) :: trackcount !Phonon Tracking array
...
22    FORMAT (f6.1,2(i5,f6.2,f5.2),2x,2f9.6,2x,2f9.6)
      ...
      write(6,*) ' ',nx,nlay,nt      
      allocate(trackcount(nx,nlay)) !,nt))              !! Allocate memory for tracking number of phonons in each area
	...
	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (iz == 1) then                      !IF QUAKE STARTS AT SURF GO DOWN
       ...
       do while ((t < t2).AND.(NITR < 100*nlay))!TRACE UNTIL TIME PASSES TIME WINDOW
			...
			ix = nint((abs(x)/deg2km-x1)/dxi) + 1
			trackcount(ix,iz) = trackcount(ix,iz) + 1.		!+1 at phonon's location
!			write(6,*) nx,nlay
			...
			do kk = 1,nlay																							   !
					...
					if (kk == 1) then																					 !
						...
						normfactor = nx*pi/360*((z_s(kk))**2-(z_s(kk-1))**2)			 !
																																		 ...
																																		 !	
					trackcount(1:nx,kk) =	trackcount(1:nx,kk) / normfactor !
			end do																												 !	
			!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!						write(17,FMT=878) (x1+real(ix-1)*dxi),z(iz),mm,trackcount(kk,ll,mm)
