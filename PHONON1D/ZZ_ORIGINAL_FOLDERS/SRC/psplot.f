...
c PSFILE_SHORT opens a postscript file with name filename
c   This version uses a shorter header 
...
c PSLAND switches orientation of plot to horizontal (landscape) mode.
c This should be called immediately after PSFILE
c Note:  This is hardwired for standard size paper! 
      subroutine PSLAND
...
c PSFONT specifies the current font.
c   text = 'h10' for 10-point Helvetica
c        = 'h14' for 14-point Helvetica
c        = 'hb12' for 12-point Helvetica-Bold
c        =  etc.
c   Note:  Only Helvetica (h8,h9,h10,h11,h12,h14,h18,h24) and
c          Helvetica-Bold (hb8,hb9,hb10,hb11,hb12,hb14,hb18,hb24)
c          are available with the PSFONT routine.  Other fonts
c          can be set directly (but note scaling to 1/000 inch scale)
c          with the PSPS subroutine or by adding to the header part of
c          the file defined with the PSFILE subroutine.  
...
c PSLINE sets current line width to flinewidth
...
c PSNUMB write number at current pen position
c   Inputs:  fp  =  number (real)
c            fmt =  desired format (e.g., 'f5.2')
...
c **** alternative name included to match EZXPLOT routine ***      
c PSNUM writes number at current pen position
c   Inputs:  fp  =  number (real)
c            fmt =  desired format (e.g., 'f5.2')
...
900   return
      ...
      subroutine PSAXES(xtic,xlab,xticl0,xfrmt,
     &                  ytic,ylab,yticl0,yfrmt)
      common/pscom/xpen,ypen,axplot,bxplot,ayplot,byplot,
     &            xpen0,ypen0,nlorg,xfrm1,xfrm2,yfrm1,yfrm2,
     &            linecount,textangle
...
c PSLAX puts x and y axes labels on axes
c   Inputs:  xlabel = x-axis label
c            xoff   = inches to offset label
c            etc.
...
c PSTIT puts title above axes
c   Inputs:  title = title
c            yoff   = inches to offset title from top
...
c PSNOTE writes text in lower left corner of plot
...
c PSSYMB plots symbols at current pen position
c   n   = 1  --  +
c       = 2  --  square
c       = 3  --  triangle
c       = 4  --  inverted triangle
c       = 5  --  X
c       = 6  --  diamond
c       = 7  --  hexagon
c       = 8  --  circle
c size2 = symbol height in inches
c NOTE:  Negative symbol numbers make filled symbols
...
c PSBOX draws filled in grayshade box (gray=0 for black, =1 for white)
...
c PSCBOX draws filled in color box
...
c PSIMAG displays a bitmap 
c     ibuf  =  integer*2 array containing bitmap
c              1 number per pixel for grayscale
c              3 numbers per pixel for color (RBG multiplexed)
c              values must be between 0 and 255
c              1st pixel is at lower left, last at upper right
c              image is formed by rows from bottom to top
c     nx    =  number of columns of ibuf (pixels)
c     ny    =  number of rows of ibuf (pixels)
c     icol  =  0 for grayscale
c           =  1 for color 
