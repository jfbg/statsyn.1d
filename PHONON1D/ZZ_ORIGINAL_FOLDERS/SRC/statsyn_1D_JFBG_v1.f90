...
!			DOUBLE PRECISION,ALLOCATABLE,DIMENSION(:,:) :: trackcount !Phonon Tracking array	
      ...
      INTEGER     :: ix,nx    !! Index & number of distances
      ...
      REAL        :: pow2,pow1 !! Normalization factor for hilbert transform
...
!      CALL srand(idum)
      ...
      WRITE(6,'(A)') 'ENTER TRACK COUNT OUTPUT FILE:'
      READ (5,'(A)')  tfile 
			WRITE(6,*) 'Tfile:',tfile
...
22    FORMAT (f6.1,2(i5,f6.2,f5.2),2x,2f9.6,2x,2f9.6)
			...
			WRITE(6,*) '!!!!!!!!!!!!!!!!!!!!!'
			WRITE(6,*) ' ',nx,nlay,nttrack
      allocate(trackcount(nx,nlay,nttrack))              !! Allocate memory for tracking number of phonons in each area
      ...
      WRITE(6,*) 'BEGINNING RANDOM TRACE:',nts
       ...
       CALL srand(seed)
	     ...
	     r0 = rand()							  !First rand output not random
			 ...
			 minattn = 0.
			 do JJ = 1,nts
						minattn = minattn + (2*mts(2,1,JJ))**2
			 end do
			 
			 WRITE(6,*) '!!!!!!!!!!!!!! ==> ', minattn

	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	!!!!! START OF RAY TRACING LOOP
	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			 OPEN(73,file='timevalues',status='unknown')		!JFBG
			...
			!SET UP INITIAL PARAMETERS (takeoff angle, P, SV, SH, ...)
			 iz = iz1
...
!			WRITE(6,*) ip,r0 !JFBG
      ...
      if (iz == 1) then                       !IF QUAKE STARTS AT SURF GO DOWN
       ...
       do while ((t < t2).AND.(NITR < 100*nlay))!TRACE UNTIL TIME PASSES TIME WINDOW
				...
				if (t-t_last > 0) then
					ixtrack = nint((abs(x)/deg2km-x1)/dxi) + 1
					itt = nint(t/REAL(nttrack_dt)+.5)
						
				!WRITE(6,*) t, t_last !JFBG
...
!         ims = 2
					...
					! Do total power/energy of source at time t
...
!					WRITE(6,*) t, attn, frac, ims, icaust !JFBG
					...
					trackcount(ixtrack,iztrack,itt) = trackcount(ixtrack,iztrack,itt) + attn/minattn!*(t-t_last)		!+1 at phonon's location
				end if

				t_last = t
				
! 		if (I == 1) then				!JFBG
!				WRITE(73,*) ix,iz,itt,t
!			end if
       ...
       WRITE(22,FMT=888) 999.99,(x1+dxi*float(J-1),J=1,nx)
		  ...
		  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			...
			do kk = 2,nlay																							   !
																																		 !
					if (kk == 2) then																					 !
						normfactor = nx*pi/360*(z_s(kk))**2											 !
					else																											 !
						normfactor = nx*pi/360*((z_s(kk))**2-(z_s(kk-1))**2)     !
																																		 !
					end if																										 !
					
					if (normfactor == 0) cycle																													 																													 !	
					trackcount(1:nx,kk,1:nttrack) =	trackcount(1:nx,kk,1:nttrack) / normfactor !
			end do																												 !	
			!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			
			
			!! WRITE up file for plotting !!
