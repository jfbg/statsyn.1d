...
!      ----- DECLARATIONS ----
        ...
        INTEGER       SourceTYPE,samplingtype,sI,distcount(nx0)
        ...
        INTEGER   Cnp_SH,Cnp1_SH,CI_SH,Cindex_SH,ip_ori
...
!      OPEN(77,FILE='Debug_x.txt',STATUS='UNKNOWN')    !OPEN OUTPUT FILE
      ...
      OPEN(76,FILE='Track_imth.txt',STATUS='UNKNOWN')    !OPEN OUTPUT FILE
...
!       WRITE(6,*) '       Prescat :',tt3-tt2,I
            ...
            dh = z(iz) - z(iz-1) !First dh is thickness of layer (FLAT)
               ...
               ! Calculate linear distance to next velocity layer (ds_SL)
                 ...
                 CALL LAYERTRACE(p,dh,utop,ubot,imth,dx1,dt1,irtr1,iz)
        ...
        ! RECORD IF PHONON IS AT SURFACE
         IF (iz == 1) THEN
          ...
          IF ( abs(xo-x_index/deg2km) <= dreceiver) THEN
          				...
          				!DEBUG
          				!Add one each time there is a hit. Hit should could down as 1/r due to geometrical spreading
!                  distcount(ix) = distcount(ix) + 1
...
!                    END IF
          ...
          END IF
 					...
 					!DEBUGTRACKPARAM
					ang1 = asin(p*vf(1,iwave))
          WRITE(76,*) p,ix,x_index/deg2km,ip_ori,anginit,x,ang1
...
subroutine LAYERXT(p,h,utop,ubot,imth,dx,dt,irtr)
!       LAYERTRACE_ORI(p,h,utop,ubot,imth,dx,dt,irtr)

      IMPLICIT NONE
      REAL(8)   p,h,utop,ubot,dx,dt
      INTEGER   imth,irtr
      REAL(8)   u,y,q,qs,qr,vtop,vbot,etau,ex,z,dtau
      REAL(8)   u1,u2,v1,v2,b,eta1,tau1,eta2,tau2,x1,x2
      ...
      return
END SUBROUTINE LAYERXT

SUBROUTINE LAYERTRACE(p,h,utop,ubot,imth,dx,dt,irtr,iz)
!   | --- --------- --------- --------- --------- --------- --------- -- !   !
...
!      init_ud = ud
        ...
        IF (iz /= 1) THEN
          ...
          CALL LAYERTRACE(p,h,utop,ubot,imth,dx1,dt1,irtr1,iz)
...
SUBROUTINE RAYTRACE_SCAT
      ...
      END IF 
          ...
          CALL LAYERTRACE(p,dh,utop,ubot,imth,dx1,dt1,irtr1,iz_scat)
...
SUBROUTINE GET_DS_SL
      ...
      END IF 
       ...
       CALL LAYERTRACE(p,dh_temp,utop,ubot,imth,dx1,dt1,irtr1,iz)
