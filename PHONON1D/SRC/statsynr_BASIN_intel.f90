...
!      ----- DECLARATIONS ----
        ...
        INTEGER, PARAMETER :: nt0=190000, nx0=1441,ns0=2001,nst0=4000
...
60    WRITE(6,'(A)') 'ENTER DISTANCE RANGE (x1, x2, nx IN DEGREES):'
      ...
      WRITE(6,'(A)') 'ENTER BASIN TYPE (SEE LIST)'!FOR TRACKING IN TERMINAL WHEN MANY KERNELS ARE RUNNING AT ONCE
      READ (5,*)  basintype
      WRITE(6,*) 'BASINTYPE =',basintype
...
!       WRITE(6,*) '       Prescat :',tt3-tt2,I
      ...
      !                        IF (irtr1 /= 0)  z_act = z_act + dh*ud !Calculate new depth FLAT
                              ...
                              CALL CHECKSCATPROB   !Check if near non-scattering basin
...
SUBROUTINE CHECKSCATPROB
      ...
      x_index = abs(x)
      x_index = mod(x_index,circum)
      IF (x_index >= circum/2) x_index = x_index - 2*(x_index-circum/2)
      
      xd = x_index/deg2km    !in degrees
      
      scat_prob2 = scat_prob
      
      IF (basintype.eq.0) THEN
        scat_prob2 = scat_prob
        
      ELSEIF (basintype.eq.1) THEN
        ! BASIN001
        ...
        ! Source is in similar basin with no scattering if impact.
			 ...
			 ELSEIF (basintype.eq.2) THEN
							! BASIN002
							! Two 20deg rad basins, see effect of near edge receivers, no scat in basin
							basrad = 20.
        ...
        ! Scat only if z_scat < 5km
							...
							END IF
			ELSE
			  !Reset to SL_prob value if out of basin
			  scat_prob2 = scat_prob
			  	
			END IF  !(basintype.eq.)
			
			!DEBUG
!			WRITE(6,*) x_index,xd,scat_prob2,I,t,z_act,p
